...
const fetchNovels = useCallback(async () => {
  ...
  const handleAiChapterUpdated = useCallback(() => {
    setChapterReloadToken((prev) => prev + 1)
    refreshNovelSnapshot()
  }, [refreshNovelSnapshot])
        ...
        {navView === 'library' ? (
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
              <header className="border-b border-border/40 bg-white/80 backdrop-blur-xl px-6 py-4 shadow-soft">
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                    <div className="space-y-2">
                      ...
                      <p className="hidden max-w-2xl text-sm text-muted-foreground leading-relaxed lg:block">
                    ...
                    {TOOL_DEFINITIONS.map((tool) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            'flex shrink-0 items-center gap-2 whitespace-nowrap rounded-full px-4 py-2.5 text-sm font-semibold transition-all duration-200',
                            active
                              ? 'bg-gradient-to-br from-primary to-teal-600 text-white shadow-card'
              ...
              <main className="flex-1 overflow-y-auto px-6 pb-8 pt-4">
                {selectedNovelId && selectedChapterId ? (
                  <ChapterComposer
                    novelId={selectedNovelId}
                    chapterId={selectedChapterId}
                    reloadToken={chapterReloadToken}
            ...
            <aside
              className={cn(
                'hidden border-l border-border/60 bg-card/60 transition-all duration-300 lg:block',
                assistantOpen
                  ? 'w-[360px] overflow-y-auto p-4 opacity-100 xl:w-[420px]'
                  : 'w-0 overflow-hidden p-0 opacity-0 pointer-events-none border-l-0'
              )}
            >
